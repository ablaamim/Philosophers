		...
		**forks, t_philo **philosophers)
{
	...
	*philosophers = (t_philo *) malloc (sizeof(t_philo) * n_of_philos);
	...
	while (++i < n_of_philos)
	{
		...
		(*philosophers)[i].supper_locker = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t) * 0x1);
		(*philosophers)[i].meals_locker = (pthread_mutex_t *) malloc(sizeof(pthread_mutex_t) * 0x1);
